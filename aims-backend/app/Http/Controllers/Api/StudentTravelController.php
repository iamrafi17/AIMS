...
class StudentTravelController extends Controller
{
    ...
    public function start(Request $request, TravelLog $travel)
    {
        ...
        if ($travel->status !== 'scheduled') throw ValidationException::withMessages(['travel' => 'Only scheduled travel can be started.']);
    ...
    public function checkpoint(Request $request, TravelCheckpoint $checkpoint)
    {
        ...
        if ($request->hasFile('photo')) {
            if ($checkpoint->photo_path) Storage::disk('public')->delete($checkpoint->photo_path);
